...
function TestSql() {
	var a = 'coal';
	db.all("SELECT "+a+",state_abbreviation FROM Consumption ORDER BY state_abbreviation,year", (err, rows) => {
		...
		for(var i=0; i<bag.length; i++) {
			bag[i] = new Array(2);
			bag[i][0] = rows[i*(rows.length/51)].state_abbreviation;
			for(var k=0; k<(rows.length/51); k++) {
				var temp = new Array(rows.length/51);
				temp[k] = rows[(i*(rows.length/51))+k][a];
				bag[i][1] = temp;
			}
		}
